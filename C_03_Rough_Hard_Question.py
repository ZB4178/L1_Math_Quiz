...
while wanted == "yes":
    ...
    if constant == 0:
        constant = 1
    variable = random.randint(1,12)
    times_variable = random.randint(2,8)
    answer = times_variable * (variable + constant)
    ...
    if constant <= 0:
        constant *= -1
        user_answer = int_check(f"\nIf {times_variable}({placeholder} - {constant}) = "
                                f"{answer}, what is '{placeholder}'? ",
                                low=1, exit_code="xxx")
    else:
        user_answer = int_check(f"\nIf {times_variable}({placeholder} + {constant}) = "
                                f"{answer}, what is '{placeholder}'? ",
